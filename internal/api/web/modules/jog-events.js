...
export function createJogEventHandler({ state, documentRef, performanceRef, callbacks = {} } = {}) {
  const { flushQueuedTapMoveArm, resetJogInputSender, clearDisarmedMovementState, clearNotice, reconcileObservedMachineStatus, mergeMachineStatusForDisplay, resolveOutlineCaptureIntent, tapMoveArmSuccessText, requestMovementDisarm, completeCommandDisarm, finishSurfaceMPGGesture, setStatusMessage, beginOriginVerification, clearOriginVerification, setOriginFeedback, handleOriginAck, completeWorkCoordinateMove, clearFieldProbeMove, renderJog, renderOutlineCapture, jogErrorText, tapMoveArmFailureText, cancelWorkCoordinateMove, originTargetLabel, hasPendingOriginOperation, deferSurfaceMPGMachineRender, renderSurfaceMPGWheel, renderMachine } = callbacks;

  function applyJogEvent(ev) {  let machineChanged = false;
