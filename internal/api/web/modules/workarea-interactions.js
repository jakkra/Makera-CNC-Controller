export function createWorkAreaInteractions({ state, documentRef = globalThis.document, windowRef = globalThis, constants = {}, callbacks = {} }) {
  const { WORKAREA_PAN_THRESHOLD_PX, WORKAREA_ZOOM_STEP, MOBILE_WORKAREA_MAX_WIDTH_PX } = constants;
  const {
  ...
  function mobileWorkAreaJogEnabled() {
    return isMobileWorkAreaJogEnabled(windowRef, MOBILE_WORKAREA_MAX_WIDTH_PX);
  }

  function mobileWorkAreaActionsOpen() {
    return !!documentRef.getElementById("workarea-actions-panel")?.classList.contains("is-open");
  }
  ...
  function setMobileWorkAreaJogVisual(origin, knob, radiusPX) {
    ...
    const group = documentRef.getElementById("workarea-mobile-jog");
  ...
  function bindLifecycleInteractions({
    mobileWorkAreaJogEnabled: mobileEnabled = mobileWorkAreaJogEnabled,
    releaseJogInput: release = releaseJogInput,
    renderJog: render = renderJog,
  } = {}) {
    windowRef?.addEventListener?.("resize", () => {
      if (!mobileEnabled() && state.workarea?.mobileJogActive) {
        if (release(true)) render();
      }
    });
  }
  ...
  function bindWorkAreaInteractions() {
    ...
    windowRef.addEventListener("pointerup", stopMobileWorkAreaJog);
    windowRef.addEventListener("pointercancel", stopMobileWorkAreaJog);
  ...
  return {
    handleWorkAreaTap, mobileWorkAreaJogEnabled, mobileWorkAreaActionsOpen,
    ...
    handleWorkAreaPointerUp, handleWorkAreaWheel, bindZoomInteractions, bindLifecycleInteractions, bindWorkAreaInteractions,
  };
}
