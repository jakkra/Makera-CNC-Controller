...
func TestWebUIServed(t *testing.T) {
	srv, _ := newTestServer(t)
	for _, route := range []string{"/dashboard", "/active-job", "/jog", "/control", "/files", "/maintenance"} {
	...
	for _, want := range []string{
		`<canvas id="dashboard-preview" role="img" aria-label="Active job preview"></canvas>`,
	...
	for _, gone := range []string{`id="dashboard-preview-path"`, `id="dashboard-preview-complete"`, `id="dashboard-preview-marker"`, `aria-label="Active job top view"`, `id="dashboard-open-job"`} {
		if strings.Contains(bodyText, gone) {
			t.Errorf("index still contains obsolete dashboard 2D preview marker %s", gone)
		}
	}
	for _, want := range []string{`[hidden] { display: none !important; }`, `id="status-bar"`, `.status-item`, `#status-bar { position: fixed;`, `background: transparent; border: 0; box-shadow: none; pointer-events: none;`, `.status-item.entering`, `.status-item.leaving`, `.status-dismiss`, `.jobs-head`, `.job-recovery`, `id="machine-status-toolbar"`, `id="machine-status-popout"`, `class="connection-status"`, `id="header-toggle"`, `aria-label="Hide top bars"`, `body.header-collapsed #command-toolbar, body.header-collapsed .tabs { display: none; }`, `id="alarm-panel"`, `id="alarm-recover"`, `data-control-action="recover"`, `id="ctl-home-main"`, `data-control-action="home"`, `id="dashboard-view"`, `id="dashboard-state"`, `id="machine-readout-template"`, `data-machine-readout-host`, `data-machine-space="work"`, `data-machine-space="machine"`, `data-machine-metric="feed"`, `data-machine-feed-override`, `data-machine-metric="spindle"`, `data-machine-metric="tool"`, `id="dashboard-external-camera"`, `id="dashboard-external-camera-zoom-in"`, `id="dashboard-external-camera-zoom-center"`, `id="dashboard-builtin-camera"`, `id="dashboard-preview"`, `id="dashboard-progress-bar"`, `id="dashboard-remaining"`, `id="active-job-view"`, `id="active-gcode-left"`, `id="active-job-left-tab-source"`, `id="active-job-left-tab-console"`, `id="active-gcode-console"`, `id="active-gcode-splitter"`, `role="separator"`, `aria-orientation="vertical"`, `id="gcode-form"`, `id="gcode-input"`, `id="log-filter"`, `id="tool-panel"`, `id="tool-set"`, `id="tool-change-select"`, `id="tool-continue"`, `Tool Status`, `id="active-gcode-panel"`, `class="active-gcode-head"`, `id="active-gcode-progress"`, `id="active-gcode-elapsed"`, `id="active-gcode-remaining"`, `id="active-gcode-pause"`, `id="active-gcode-resume"`, `id="feed-override-controls"`, `id="feed-override-decrease"`, `id="feed-override-increase"`, `id="feed-override-reset"`, `id="paused-job-raise"`, `id="paused-job-stop-spindle"`, `id="active-gcode-source"`, `id="active-gcode-source-scroll"`, `id="active-gcode-source-position"`, `id="gcode-preview"`, `id="gcode-timeline"`, `id="gcode-projection-persp" aria-pressed="false"`, `id="gcode-projection-ortho" aria-pressed="true"`, `id="jog-settings-section"`, `id="move-to-work-section"`, `id="work-zero-section"`, `id="gamepad-section"`, `id="workarea-mobile-jog"`, `.mobile-jog-base`, `.mobile-jog-knob`, `class="command-panel-close" aria-label="Close Macros menu"`, `class="command-panel-close" aria-label="Close Tool menu"`, `type="module"`, `/app.js?v=overview-feed-override-1`} {
		if !strings.Contains(bodyText, want) {
	...
	for _, want := range []string{`id="dashboard-external-camera-snapshot"`, `id="dashboard-camera-snapshot-modal"`, `id="dashboard-camera-snapshot-image"`} {
		if !strings.Contains(bodyText, want) {
			t.Errorf("index missing camera snapshot marker %s", want)
		}
	}
	for _, want := range []string{`id="dashboard-external-camera-focus-open"`, `id="dashboard-camera-focus-modal"`, `id="dashboard-camera-focus-mode"`, `id="dashboard-camera-focus-value"`} {
		if !strings.Contains(bodyText, want) {
			t.Errorf("index missing camera focus marker %s", want)
		}
	}
	...
	for _, want := range []string{
		...
		`#files .actions { display: flex; grid-area: actions; justify-content: flex-end; gap: 6px;`,
	...
	activeJobLayoutModule := get(t, srv.URL+"/modules/active-job-layout.js")
	activeJobLayoutBody, _ := io.ReadAll(activeJobLayoutModule.Body)
	activeJobLayoutModule.Body.Close()
	...
	dashboardViewModule := get(t, srv.URL+"/modules/dashboard-view.js")
	dashboardViewBody, _ := io.ReadAll(dashboardViewModule.Body)
	dashboardViewModule.Body.Close()
	...
	workMoveModule := get(t, srv.URL+"/modules/work-move.js")
	workMoveBody, _ := io.ReadAll(workMoveModule.Body)
	workMoveModule.Body.Close()
	...
	outlineDXFModule := get(t, srv.URL+"/modules/outline-dxf.js")
	outlineDXFBody, _ := io.ReadAll(outlineDXFModule.Body)
	outlineDXFModule.Body.Close()
	...
	heightMeshModule := get(t, srv.URL+"/modules/height-mesh.js")
	heightMeshBody, _ := io.ReadAll(heightMeshModule.Body)
	heightMeshModule.Body.Close()
	heightTriangulationModule := get(t, srv.URL+"/modules/height-triangulation.js")
	heightTriangulationBody, _ := io.ReadAll(heightTriangulationModule.Body)
	heightTriangulationModule.Body.Close()
	heightCoordinatesModule := get(t, srv.URL+"/modules/height-coordinates.js")
	heightCoordinatesBody, _ := io.ReadAll(heightCoordinatesModule.Body)
	heightCoordinatesModule.Body.Close()
	...
	outlineCaptureOperationsModule := get(t, srv.URL+"/modules/outline-capture-operations.js")
	outlineCaptureOperationsBody, _ := io.ReadAll(outlineCaptureOperationsModule.Body)
	outlineCaptureOperationsModule.Body.Close()
	webSource := string(jsBody) + string(activeJobViewBody) + string(activeJobLayoutBody) + string(toolBody) + string(originBody) + string(filesModuleBody) + string(gcodeModuleBody) + string(machineStatusBody) + string(dashboardTelemetryBody) + string(dashboardViewBody) + string(gcodeLogBody) + string(navigationBody) + string(surfaceJogBody) + string(jogBody) + string(jogViewBody) + string(jogEventsBody) + string(settingsBody) + string(workareaBody) + string(workMoveBody) + string(outlineBody) + string(outlineIOBody) + string(outlineCaptureBody) + string(outlineDXFBody) + string(outlineFilesBody) + string(heightExportBody) + string(heightMeshBody) + string(heightTriangulationBody) + string(heightCoordinatesBody) + string(commandUIBody) + string(workareaJogBody) + string(fieldProbingBody) + string(outlineCaptureOperationsBody)
	...
	for _, want := range []string{`from "./modules/api.js"`, `from "./modules/dom.js"`, `from "./modules/format.js"`, `from "./modules/maintenance.js"`, `from "./modules/files.js"`, `from "./modules/active-job.js"`, `from "./modules/active-job-layout.js"`, `from "./modules/active-job-view.js"`, `from "./modules/camera.js"`, `from "./modules/dashboard-telemetry.js"`, `from "./modules/dashboard-view.js"`, `from "./modules/gcode-log.js"`, `from "./modules/navigation.js"`, `from "./modules/surface-routing.js"`, `from "./modules/surface-shell.js"`, `from "./modules/machine-reconciliation.js"`, `from "./modules/machine-commands.js"`, `from "./modules/surface-actions.js"`, `from "./modules/feature-state-ports.js"`, `from "./modules/outline-coordination.js"`, `from "./modules/outline-io.js"`, `from "./modules/outline-capture.js"`, `from "./modules/outline-dxf.js"`, `from "./modules/outline-files.js"`, `from "./modules/height-export.js"`, `from "./modules/height-mesh.js"`, `from "./modules/height-triangulation.js"`, `from "./modules/command-ui.js"`, `from "./modules/workarea-jog.js"`, `from "./modules/jog.js"`, `from "./modules/jog-view.js"`, `from "./modules/jog-events.js"`, `from "./modules/gamepad-controls.js"`, `from "./modules/surface-controls.js"`, `from "./modules/workarea-interactions.js"`, `from "./modules/work-move.js"`, `from "./modules/field-probing.js"`, `from "./modules/outline-capture-operations.js"`, `from "./modules/state.js"`} {
	...
	for _, asset := range []struct {
		...
		{"/modules/dom.js", "export function bindButtonAction"},
		{"/modules/feature-state-ports.js", "export function createFeatureStatePorts"},
		...
		{"/modules/active-job-layout.js", "function bindInteractions"},
		...
		{"/modules/machine-commands.js", "export function createMachineCommands"},
		{"/modules/surface-actions.js", "export function createSurfaceActions"},
		...
		{"/modules/active-job.js", "export function bindActiveJobInteractions"},
		{"/modules/active-job.js", "export function mountActiveJobDispatch"},
		...
		{"/modules/dashboard-profiles.js", "function bindResponsiveInteractions"},
		...
		{"/modules/outline.js", "function closeOutline()"},
		{"/modules/outline.js", "function undoOutline()"},
		{"/modules/outline.js", "function redoOutline()"},
		...
		{"/modules/jog.js", "bindJogArmInteractions"},
		...
		{"/modules/surface-controls.js", "function bindFooterInteractions"},
		...
		{"/modules/outline-coordination.js", "export function createOutlineCoordination"},
	...
	for _, check := range []struct {
		...
		{webSource, "renderWorkArea"}, {webSource, "SPINDLE_DIAMETER_MM"}, {webSource, "OUTLINE_POINT_DIAMETER_MM"},
		{webSource, "jogPanelMessage"}, {webSource, "sendTapMove"}, {webSource, "sendWorkCoordinateMove"},
		{webSource, "workMoveTargetsFromInputs"}, {webSource, "resetWorkMoveInput"}, {webSource, "workMoveInputIsLive"},
		{webSource, "renderWorkMoveFieldState"}, {webSource, "stepTapFeed"}, {webSource, "feedBoundsFor"}, {webSource, "stepZ"},
		{originSource, "setOriginAxis"}, {originSource, "applyXYZOrigin"}, {originSource, "applyOriginSource"},
		{originSource, "originTargetsFromOriginSource"}, {originSource, "originTargetsFromXYZ"}, {originSource, "machineAnchorPoints"},
		{originSource, "runAutoZProbe"}, {originSource, "recallSelectedOrigin"}, {originSource, "saveCurrentOrigin"},
		{originSource, "deleteSelectedOrigin"}, {originSource, "saved_origins"}, {webSource, `type: "target"`},
		{webSource, `type: "step"`}, {originSource, `type: "origin"`}, {originSource, "G10L20P0"},
		{originSource, "/api/probe/auto-z"}, {string(liveBody), "/api/machine/status"},
		{string(jsBody) + string(outlineCaptureOperationsBody), "motion_estimated"},
	} {
		if !strings.Contains(check.source, check.mark) {
			t.Errorf("web UI missing work area or machine contract marker %q", check.mark)
		}
	}
	...
	for _, want := range []string{"/api/files/retry", "/api/files/discard"} {
		if !strings.Contains(string(filesModuleBody), want) {
			t.Errorf("modules/files.js missing %s", want)
		}
	}
